...
// Vérifie si un niveau événement est actuellement actif
function isEventActive(level) {
  if (level.type !== "event") return false;
  const today = new Date().toISOString().slice(0, 10);
  return today >= level.eventStart && today <= level.eventEnd;
}

// Vérifie si un niveau événement est terminé
function isEventOver(level) {
  if (level.type !== "event") return false;
  const today = new Date().toISOString().slice(0, 10);
  return today > level.eventEnd;
}

// Groupe les niveaux event par eventId
...
// Miniature pixel art d'un niveau
function LevelThumb({ level, size = 4 }) {
...
const CatalogueScreen = memo(function CatalogueScreen({ catalogue, onClose, onPlay }) {
  const [tab, setTab] = useState("standard"); // "standard" | "event"
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button style={tabStyle(tab === "standard")} onClick={() => setTab("standard")}>
            STANDARDS
          </button>
          <button style={tabStyle(tab === "event")} onClick={() => setTab("event")}>
            ÉVÉNEMENTS
          </button>
        </div>
          ...
          {tab === "standard" && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
              {standardLevels.map(lvl => {
                ...
                const done = stars >= 0;
                // Taille miniature adaptée : grilles plus grandes = pixels plus petits
                const thumbSize = lvl.cols > 10 ? 2 : 4;
                return (
                  <div key={lvl.id} onClick={() => done && onPlay(lvl.id)} style={{
                    ...
                    {done ? (
                      <LevelThumb level={lvl} size={thumbSize} />
                    ) : (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {eventGroups.length === 0 ? (
                <div style={{
                  ...
                  fontSize: 7, color: "#aaa", lineHeight: 2,
                }}>
                ...
                eventGroups.map(group => {
                  const today = new Date().toISOString().slice(0, 10);
                  const active = today >= group.eventStart && today <= group.eventEnd;
                  const over = today > group.eventEnd;
                  const upcoming = today < group.eventStart;

                  // Compte les niveaux complétés dans ce groupe
                  const completedCount = group.levels.filter(l => (catalogue[l.id] ?? -1) >= 0).length;
                  const allDone = completedCount === group.levels.length;
                    ...
                    <div key={group.eventId}>
                      ...
                      <div style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 6, color: "#888" }}>
                            {group.eventStart.split("-").reverse().join("/")} →{" "}
                            {group.eventEnd.split("-").reverse().join("/")}
                          </div>
                        </div>
                        <div style={{
                          fontSize: 7, padding: "4px 8px", borderRadius: 6,
                          background: active ? "rgba(65,117,5,0.15)" : over ? "rgba(0,0,0,0.08)" : "rgba(74,144,217,0.15)",
                          color: active ? "#417505" : over ? "#888" : "#4A90D9",
                          border: `1px solid ${active ? "rgba(65,117,5,0.3)" : over ? "rgba(0,0,0,0.1)" : "rgba(74,144,217,0.3)"}`,
                        }}>
                          {active ? "EN COURS" : over ? "TERMINÉ" : "À VENIR"}
                        </div>
                      </div>
                        ...
                        <div style={{ marginBottom: 10 }}>
                          <div style={{
                            ...
                            <div style={{
                              ...
                              width: `${Math.round(completedCount / group.levels.length * 100)}%`,
                              background: allDone
                          ...
                          <div style={{ fontSize: 6, color: "#888" }}>
                            {completedCount}/{group.levels.length} niveau{group.levels.length > 1 ? "x" : ""} complété{completedCount > 1 ? "s" : ""}
                            {allDone ? " 🏆" : ""}
                          </div>
                        </div>
                      )}
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10 }}>
                        {group.levels.map(lvl => {
                          const stars = catalogue[lvl.id] ?? -1;
                          const done = stars >= 0;
                          const playable = active || done; // jouable si actif OU déjà fait
                          const thumbSize = lvl.cols > 15 ? 1 : lvl.cols > 10 ? 2 : 4;
